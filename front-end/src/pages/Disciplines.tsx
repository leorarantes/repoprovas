  ...
  return (
    <>
      <Autocomplete
        ...
        options={getDisciplinesOptions(terms)}
      ...
      >
        ...
        <TermsAccordions categories={categories} terms={terms} disciplineName={discipline} />
      </Box>
    </>
  );
}
...
function TermsAccordions({ categories, terms, disciplineName }: TermsAccordionsProps) {
  if (disciplineName) {
    return (
      <Box sx={{ marginTop: "50px" }}>
        <DisciplinesAccordions
          categories={categories}
          disciplines={terms.filter((term) => term.disciplines.some((discipline) => discipline.name === disciplineName))[0].disciplines.filter((discipline) => discipline.name === disciplineName)}
        />
      </Box>
    );
  }
...
function DisciplinesAccordions({ categories, disciplines }: DisciplinesAccordionsProps) {
...
function getDisciplinesOptions(terms: TestByDiscipline[]) {
  let disciplines = new Array<Discipline>();

  terms.forEach((term) => {
    term.disciplines.forEach((discipline) => disciplines.push(discipline))
  });
